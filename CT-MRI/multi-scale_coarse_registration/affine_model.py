...
class Affine_Model(object):
	...
	def affine(self, CT, MRI):
		self.CT = CT
		self.MRI = MRI

		with tf.device('/gpu:0'):
			self.CT_d4 = img_resize(self.CT, 0.25)
			self.MRI_d4 = img_resize(self.MRI, 0.25)
			self.CT_d2 = img_resize(self.CT, 0.5)
			self.MRI_d2 = img_resize(self.MRI, 0.5)

			self.affine_net_d4 = Affine_Generator('affine_net_d4', self.is_training)
			dtheta_d4 = self.affine_net_d4.field_model(self.CT_d4, self.MRI_d4)

			self.affine_net_d2 = Affine_Generator('affine_net_d2', self.is_training)
			self.warped_CT_d2, label2, _ = apply_affine_trans(self.CT_d2, dtheta_d4)
			self.warped_CT_d2 = tf.multiply(self.warped_CT_d2, tf.tile(label2, [1, 1, 1, 3]))
			dtheta_d2 = self.affine_net_d2.field_model(self.warped_CT_d2, self.MRI_d2)

			self.affine_net = Affine_Generator('affine_net', self.is_training)
			self.warped_CT_d1, label1, _ = apply_affine_trans(self.CT, multi_affine(dtheta_d4, dtheta_d2))
			self.warped_CT_d1 = tf.multiply(self.warped_CT_d1, tf.tile(label1, [1, 1, 1, 3]))
			dtheta_d1 = self.affine_net.field_model(self.warped_CT_d1, self.MRI)

			self.dtheta = multi_affine(multi_affine(dtheta_d4, dtheta_d2), dtheta_d1)

			self.warped_CT, self.label, self.resampling_grid = apply_affine_trans(self.CT, self.dtheta)
			self.warped_CT = tf.multiply(self.warped_CT, tf.tile(self.label, [1, 1, 1, 3]))

			self.fuse_before = (self.CT + tf.tile(self.MRI, [1, 1, 1, 3]))/2
			self.fuse_after = (self.warped_CT + tf.tile(self.MRI, [1, 1, 1, 3]))/2


		if self.is_training:
			with tf.device('/gpu:1'):
				self.extract_model = Des_Extract_Model(self.batchsize, self.INPUT_H, self.INPUT_W, is_training=False, equivariance=False)
				self.extract_model.des(self.CT, self.MRI)
				self.CT_des = self.extract_model.CT_des
				self.MRI_des = self.extract_model.MRI_des
				self.CT_des, self.MRI_des = normalize_common_tf(self.CT_des, self.MRI_des)

			self.warped_CT_des, _, _, _ = grid_sample(self.CT_des, self.resampling_grid)
			_, label_d4, resampling_grid_d4 = apply_affine_trans(self.CT, dtheta_d4)
			self.warped_CT_des_d4, _, _, _ = grid_sample(self.CT_des, resampling_grid_d4)

			self.w_des1 = tf.multiply(self.warped_CT_des, self.label)
			self.w_des2 = tf.multiply(self.MRI_des, self.label)

			self.w_des1_d4 = tf.multiply(self.warped_CT_des_d4, label_d4)
			self.w_des2_d4 = tf.multiply(self.MRI_des, label_d4)
	...
	def field_model(self, img_a, img_b):
		with tf.variable_scope(self.scope):
			...
			with tf.variable_scope('layer1'):
				weights = tf.get_variable("w1", [9, 9, 2, 16],
