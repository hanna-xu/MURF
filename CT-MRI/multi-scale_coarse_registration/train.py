...
def train_affine_network(model, sess, trainset, merged1=None, writer=None, saver=None, save_path=None, EPOCHES=20,TASK1_MODEL_PATH=None):
	...
	for epoch in range(EPOCHES):
		...
		for batch in range(n_batches):
			model.step += 1
			CT_batch = trainset[batch * model.batchsize:(batch * model.batchsize + model.batchsize), :, :, 0]
			CT_batch = np.expand_dims(CT_batch,-1)
			MRI_batch = trainset[batch * model.batchsize:(batch * model.batchsize + model.batchsize), :, :, 1]
