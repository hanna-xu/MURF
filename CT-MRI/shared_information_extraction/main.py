...
def main():
	with tf.Graph().as_default(), tf.compat.v1.Session() as sess:
		...
		CT_input = tf.compat.v1.placeholder(tf.float32, shape=(model.batchsize, patch_size, patch_size, 1), name='CT')
		MRI_input = tf.compat.v1.placeholder(tf.float32, shape=(model.batchsize, patch_size, patch_size, 1), name='MRI')
		model.des(CT_input, MRI_input)
		...
		des1 = tf.compat.v1.summary.image('des_CT', model.CT_des, max_outputs=NUM)
