...
class Des_Extract_Model(object):
	...
	def des(self, CT, MRI):
		...
		if self.equivariance:
			...
			self.data_CT = batch_rotate_p4(CT, random_rotA)
		...
		with tf.compat.v1.variable_scope('des_extract'):
			self.E_CT = Encoder('CT_Encoder')
			self.CT_des, self.F_CT = self.E_CT.encode(self.data_CT, reuse=False)
			self.E_MRI = Encoder('MRI_Encoder')
			self.MRI_des, self.F_MRI = self.E_MRI.encode(self.data_MRI, reuse=False)


		if self.equivariance:
			self.CT_des = batch_rotate_p4(self.CT_des, -random_rotA)
		...
		if self.is_training:
			...
			all_samples = tf.concat([self.CT_des, self.MRI_des], axis=0)
			...
			self.decay_loss = activation_decay([self.CT_des, self.MRI_des], p=2.) + activation_decay([self.CT_des, self.MRI_des], p=1.)
			self.grad_loss = l2_loss(tf.abs(grad(ds(self.CT_des))), tf.abs(grad(ds(self.MRI)))) \
							 + l2_loss(tf.abs(grad(ds(self.MRI_des))), tf.abs(grad(ds(CT_y))))
...
def CT2ycbcr(img_CT):
	R = tf.expand_dims(img_CT[:, :, :, 0], axis=-1)
	G = tf.expand_dims(img_CT[:, :, :, 1], axis=-1)
	B = tf.expand_dims(img_CT[:, :, :, 2], axis=-1)
