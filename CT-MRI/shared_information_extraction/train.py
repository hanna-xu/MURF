...
def train_descriptor(model, sess, trainset, merged1=None, writer=None, saver=None, save_path=None, EPOCHES=20):
	...
	var_list_CT = tf.contrib.framework.get_trainable_variables(scope='des_extract/CT_Encoder')
	...
	for epoch in range(start_epoch, EPOCHES):
		...
		for batch in range(start_batch_id, n_batches):
			model.step += 1
			CT_batch = trainset[batch * model.batchsize:(batch * model.batchsize + model.batchsize), :, :, 1]
			CT_batch = np.expand_dims(CT_batch,-1)
			MRI_batch = trainset[batch * model.batchsize:(batch * model.batchsize + model.batchsize), :, :, 0]
			...
			FEED_DICT = {model.CT: CT_batch, model.MRI: MRI_batch, model.lr: lr}
