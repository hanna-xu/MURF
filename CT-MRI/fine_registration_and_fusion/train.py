...
def train_f2m(model, sess, trainset, merged1=None, writer=None, saver=None, save_path=None, EPOCHES=20, name=None):
	...
	could_load, checkpoint_counter = load(sess=sess, saver=tf.compat.v1.train.Saver(var_list = var_list_f2m_fuse),
	...
	for epoch in range(EPOCHES):
		...
		for batch in range(n_batches):
			model.step += 1
			CT_batch = trainset[batch * model.batchsize:(batch * model.batchsize + model.batchsize), :, :, 1:2]
			MRI_batch = trainset[batch * model.batchsize:(batch * model.batchsize + model.batchsize), :, :, 0:1]

			defor_field = create_defor_field(CT_batch, type='rigid') # rigid or non_rigid
			defor_re = defor_reverse(defor_field)
			# print("defor reverse shape: ", defor_re.shape)

			lr=0.0001

			FEED_DICT = {model.CT: CT_batch, model.MRI: MRI_batch, model.defor_field: defor_field,
