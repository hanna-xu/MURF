...
class F2M_Model(object):
	...
	def f2m(self, CT, MRI, defor_field=None, re_defor_field_gt=None, label=None, is_training=True):
		self.MRI = MRI
		self.CT = CT
		if is_training:
			with tf.device('/gpu:0'):
				...
				warped_R, _, _, _ = grid_sample(CT[0:self.batchsize, :, :, 0:1], resampling_grid)
				self.dCT = warped_R
		else:
			self.dCT = CT
		...
		with tf.device('/gpu:0'):
			...
			dCT_y=self.dCT
			CT_y=self.CT
			self.fused_img, self.offset,self.res_CT= self.f2m_net.fuse(self.dCT, self.MRI)
			
			if is_training:
				dr_defor_gt, _, _, _ = grid_sample(self.dCT[0:self.batchsize, :, :, 0:1], - (resampling_grid - identity) + identity)
				self.dCT_defor_gt = dr_defor_gt
				fimg_y=self.fused_img
				self.fimg_y = self.fused_img
				dCT_offset_y=self.res_CT
				self.sim_loss = 1 - 0.5 * SSIM(fimg_y, self.MRI) - 0.5 * SSIM(fimg_y, dCT_y)

				enhanced_gradient_dCT_y = tf.where(gradient(dCT_y) > tf.zeros_like(gradient(dCT_y)),
													tf.pow(gradient(dCT_y), 0.7), -tf.pow(-gradient(dCT_y), 0.7))
				enhanced_gradient_dCT_offset_y = tf.where(gradient(dCT_offset_y) > tf.zeros_like(gradient(dCT_offset_y)),
													tf.pow(gradient(dCT_offset_y), 0.7), -tf.pow(-gradient(dCT_offset_y), 0.7))
				...
				self.max_gradient_loss = l1(gradient(self.fimg_y), tf.where(tf.abs(gradient(dCT_offset_y)) > tf.abs(gradient(self.MRI)),
													enhanced_gradient_dCT_offset_y,  enhanced_gradient_MRI))
...
class F2M_network(object):
	...
	def fuse(self, img_a, img_b, reuse=False):
		with tf.variable_scope(self.scope, reuse=reuse):
			a_offset, self.offset = convoffset2D(img_a, img_b, scope_name='conv1_offset')
			...
			conv_a1 = tf.nn.conv2d(tf.tile(a_offset, [1, 1, 1 ,3]), weights, strides=[1, 1, 1, 1], padding='SAME') + bias
...
def convoffset2D(x, y, scope_name=None):
	x_shape = tf.shape(x)
	...
	with tf.variable_scope(scope_name):
		with tf.variable_scope('offset_conv'):
			...
			offsets1 = tf.contrib.layers.batch_norm(inputs=offsets1, activation_fn=None)
			...
			offsets2 = tf.contrib.layers.batch_norm(inputs=offsets2, activation_fn=None)
			...
			offsets3 = tf.contrib.layers.batch_norm(inputs=offsets3, activation_fn=None)
			...
			offsets5 = conv(offsets3_ds, channels=32, kernel=3, stride=1, pad=1, pad_type='reflect', scope='conv5',
							use_bias=True)
			offsets5 = tf.contrib.layers.batch_norm(inputs=offsets5, activation_fn=None)
			...
			offsets6 = tf.contrib.layers.batch_norm(inputs=offsets6, activation_fn=None)
			...
			offsets7 = tf.contrib.layers.batch_norm(inputs=offsets7, activation_fn=None)
			...
			offsets8 = tf.contrib.layers.batch_norm(inputs=offsets8, activation_fn=None)
			...
			offsets10 = tf.contrib.layers.batch_norm(inputs=offsets10, activation_fn=None)
			# offsets10 = lrelu(offsets10)

			offset_x = conv(offsets10,channels=1,kernel=3,stride=1,pad=1,pad_type='reflect',
				scope='conv12', use_bias=False)
		
			offset_y = conv(offsets10,channels=1,kernel=3,stride=1,pad=1,pad_type='reflect',
				scope='conv13', use_bias=False)
			offsets = tf.concat([offset_x,offset_y],axis=-1)
