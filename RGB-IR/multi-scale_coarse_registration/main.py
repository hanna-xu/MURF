...
task1_model_path = '../task1_shared_information_extraction/models/'
...
def main():
	with tf.Graph().as_default(), tf.compat.v1.Session() as sess:
		...
		gloss1 = tf.compat.v1.summary.scalar('NCC_loss_source', model.NCC_source_loss)
		gloss2 = tf.compat.v1.summary.scalar('NCC_loss_des', model.NCC_des_loss)
		...
		merge_summary1 = tf.compat.v1.summary.merge([SOURCE1, SOURCE2, gloss1, gloss2, w_RGB, w_des1, w_des2, label, aloss])
		...
		train_affine_network(model=model, sess=sess, trainset=source_data, merged1=merge_summary1, writer=writer1, saver=saver,
						save_path='./models/', task1_model_path=task1_model_path, EPOCHES=EPOCHES)
