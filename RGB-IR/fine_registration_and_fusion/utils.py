...
# def GroupNorm(x, G=8, eps=1e-5):
# 	N, H, W, C = x.shape
# 	x = tf.reshape(x, [tf.cast(N, tf.int32), tf.cast(H, tf.int32), tf.cast(W, tf.int32), tf.cast(G, tf.int32),
...
def create_non_rigid_defor_field(x, max_defor_num=120/np.sqrt(2)):
	b, h, w, c = x.shape
	N = 15
...
def darkchannel(x):
	...
	y = - tf.nn.max_pool(-x, ksize=[1, 5, 5, 1], strides=[1, 1, 1, 1], padding='SAME')
	return y
