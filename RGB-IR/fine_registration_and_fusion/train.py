...
def train_f2m(model, sess, trainset, merged1=None, writer=None, saver=None, save_path=None, EPOCHES=20, name=None):
	...
	with tf.control_dependencies(update_ops):
		...
		model.fuse_solver = tf.compat.v1.train.AdamOptimizer(learning_rate=model.lr, beta1=0.5, beta2=0.99).minimize(
			model.fuse_loss, var_list=var_list_f2m_fuse)
		model.offset_solver = tf.compat.v1.train.AdamOptimizer(learning_rate=model.lr, beta1=0.5, beta2=0.99).minimize(
			model.offset_loss, var_list=var_list_f2m_offset)

	initialize_uninitialized(sess)
	could_load, checkpoint_counter = load(sess=sess, saver=tf.compat.v1.train.Saver(var_list=var_list_f2m),
	...
	for epoch in range(EPOCHES):
		...
		for batch in range(n_batches):
			...
			ir_batch = trainset[batch * model.batchsize:(batch * model.batchsize + model.batchsize), :, :, 3]
			...
			lr=0.0001
			FEED_DICT = {model.RGB: rgb_batch, model.IR: ir_batch, model.defor_field: defor_field,
						 model.re_defor_field_gt: defor_re, model.lr: lr}
			if epoch < 5:
			...
			if model.step % 5 == 0 and model.step > 10:
				print("\033[0;32;40m")
				print(name + '\n')
				print("\033[0m")
