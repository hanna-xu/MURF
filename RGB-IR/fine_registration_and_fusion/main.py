...
data1_path = 'aligned_RGB_IR1.h5'
data2_path = 'aligned_RGB_IR2.h5'
...
def main():
	with tf.Graph().as_default(), tf.compat.v1.Session() as sess:
		...
		RE_DEFOR_FIELD_GT = tf.placeholder(tf.float32, shape=(model.batchsize, patch_size, patch_size, 2),
										   name='re_defor_field_gt')
		model.f2m(SOURCE_RGB, SOURCE_IR, DEFOR_FIELD, RE_DEFOR_FIELD_GT)
		...
		diff1 = tf.compat.v1.summary.image('RGB-IR', model.mean_img_before, max_outputs=NUM)
		diff2 = tf.compat.v1.summary.image('dRGB-IR', model.mean_img_after, max_outputs=NUM)
		...
		drgb_offset = tf.compat.v1.summary.image('dRGB_offset', model.a_offset, max_outputs=NUM)

		merge_summary1 = tf.compat.v1.summary.merge(
			[SOURCE1, SOURCE2, gloss, dRGB, drgb_offset, diff, diff1, diff2, fimg, loss1, loss2, loss3, loss4, ofs, dgt])
