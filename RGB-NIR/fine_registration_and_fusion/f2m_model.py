...
class F2M_Model(object):
	...
	def f2m(self, RGB, NIR, defor_field=None, re_defor_field_gt=None, label=None, is_training=True):
		self.NIR = NIR
		self.RGB = RGB
		if is_training:
			with tf.device('/gpu:0'):
				...
				warped_R, _, _, _ = grid_sample(RGB[0:self.batchsize, :, :, 0:1], resampling_grid)
				warped_G, _, _, _ = grid_sample(RGB[0:self.batchsize, :, :, 1:2], resampling_grid)
				warped_B, _, _, _ = grid_sample(RGB[0:self.batchsize, :, :, 2:3], resampling_grid)
		...
		with tf.device('/gpu:0'):
			...
			self.mean_img_before = ycbcr2rgb(tf.concat([(self.NIR + rgb_y) / 2, rgb_cb, rgb_cr], axis=-1))
			self.mean_y_before = (self.NIR + rgb_y) / 2

			self.mean_img_after = ycbcr2rgb(tf.concat([(self.NIR + drgb_y) / 2, drgb_cb, drgb_cr], axis=-1))
			self.mean_y_after = (self.NIR + drgb_y) / 2

			self.fimg_y, self.offset, self.a_offset = self.f2m_net.fuse(self.dRGB, self.NIR)
			...
			self.fused_img = ycbcr2rgb(tf.concat([en(self.fimg_y), drgb_offset_cb, drgb_offset_cr], axis=-1))

			if is_training:
				dr_defor_gt, _, _, _ = grid_sample(self.dRGB[0:self.batchsize, :, :, 0:1], self.re_defor_field_gt + identity)
				dg_defor_gt, _, _, _ = grid_sample(self.dRGB[0:self.batchsize, :, :, 1:2], self.re_defor_field_gt + identity)
				db_defor_gt, _, _, _ = grid_sample(self.dRGB[0:self.batchsize, :, :, 2:3], self.re_defor_field_gt + identity)
				self.drgb_defor_gt = tf.concat([dr_defor_gt, dg_defor_gt, db_defor_gt], axis=-1)

				self.sim_loss = 1 - 0.5 * SSIM(self.fimg_y, self.NIR) - 0.5 * SSIM(self.fimg_y, drgb_offset_y)
				enhanced_gradient_drgb_offset_y = tf.where(gradient(drgb_offset_y) > tf.zeros_like(gradient(drgb_offset_y)),
													tf.pow(gradient(drgb_offset_y), 0.7), -tf.pow(-gradient(drgb_offset_y), 0.7))
				enhanced_gradient_NIR = tf.where(gradient(self.NIR) > tf.zeros_like(gradient(self.NIR)),
												tf.pow(gradient(self.NIR), 0.7), -tf.pow(-gradient(self.NIR), 0.7))
				self.max_gradient_loss = l1(gradient(self.fimg_y), tf.where(tf.abs(gradient(drgb_offset_y)) > tf.abs(gradient(self.NIR)),
													 enhanced_gradient_drgb_offset_y,  enhanced_gradient_NIR))
