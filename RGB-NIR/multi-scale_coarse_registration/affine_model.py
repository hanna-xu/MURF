...
class Affine_Model(object):
	...
	def affine(self, RGB, NIR, dropout=True):
		...
		with tf.device('/gpu:0'):
			...
			self.NIR_d4 = img_resize(self.NIR, 0.25)
			self.RGB_d2 = img_resize(self.RGB, 0.5)
			self.NIR_d2 = img_resize(self.NIR, 0.5)

			'''original'''
			self.affine_net_d4 = Affine_Generator('affine_net_d4', self.is_training)
			dtheta_d4 = self.affine_net_d4.field_model(self.RGB_d4, self.NIR_d4, dropout=dropout)
			...
			dtheta_d2 = self.affine_net_d2.field_model(self.warped_RGB_d2, self.NIR_d2, dropout=dropout)
			...
			dtheta_d1 = self.affine_net.field_model(self.warped_RGB_d1, self.NIR, dropout=dropout)
		...
		if self.is_training:
			with tf.device('/gpu:1'):
				self.extract_model = Des_Extract_Model(self.batchsize, self.INPUT_H, self.INPUT_W, equivariance=False)
				self.extract_model.des(self.RGB, self.NIR)
				self.RGB_des = self.extract_model.RGB_des
				self.NIR_des = self.extract_model.NIR_des
				self.RGB_des, self.NIR_des = normalize_common_tf(self.RGB_des, self.NIR_des)
			...
			self.w_des2 = tf.multiply(self.NIR_des, self.label)
			...
			self.w_des2_d4 = tf.multiply(self.NIR_des, label_d4)

			self.NCC_source_loss = - NCC(0.299 * self.warped_RGB[:, :, :, 0:1] + 0.587 * self.warped_RGB[:, :, :, 1:2]
										 + 0.114 * self.warped_RGB[:, :, :, 2:3], self.NIR)
			self.NCC_des_loss = - NCC(self.w_des1, self.w_des2)

			self.loss = - NCC(self.w_des1, self.w_des2) - NCC(self.w_des1_d4, self.w_des2_d4)

			self.diff_ori = tf.abs(self.NIR - tf.expand_dims(tf.reduce_mean(self.RGB, axis=-1), axis=-1))
			self.diff_after = tf.abs(self.NIR - tf.expand_dims(tf.reduce_mean(self.warped_RGB, axis=-1), axis=-1))
