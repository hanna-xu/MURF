...
def main():
	...
	with tf.Graph().as_default(), tf.Session() as sess:
		...
		for file in files:
			...
			rgb_img_1024 = scipy.misc.imresize(rgb_img, size=(H*2, W*2))
			rgb_img_1024 = np.expand_dims(rgb_img_1024, axis=0)
			rgb_img_1024 = rgb_img_1024.astype(np.float32) / 255.0

			for p in range(20):
				sess.run(affine_model.solver, feed_dict={SOURCE_RGB_N: rgb_img_N, SOURCE_NIR_N: nir_img_N})
				# print(sess.run(affine_model.loss, feed_dict={SOURCE_RGB_N: rgb_img_N, SOURCE_NIR_N: nir_img_N}))

			warped_rgb, dtheta = sess.run([WSOURCE1, affine_model.dtheta], feed_dict={SOURCE_RGB_N: rgb_img_N, SOURCE_NIR_N: nir_img_N, SOURCE_RGB: rgb_img_1024})
