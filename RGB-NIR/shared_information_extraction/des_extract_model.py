...
class Des_Extract_Model(object):
	def __init__(self, BATCH_SIZE, INPUT_H, INPUT_W, is_training=False, equivariance=True, scope_name='des_extract'):
	...
	def des(self, RGB, NIR, reuse=False):
		self.RGB = RGB
		self.NIR = NIR
		oh = NIR.shape[1]
		ow = NIR.shape[2]
		...
		if self.equivariance:
			...
			self.data_NIR = batch_rotate_p4(NIR, random_rotB)
		else:
			self.data_RGB = RGB
			self.data_NIR = NIR
		...
		with tf.compat.v1.variable_scope(self.scope_name, reuse=reuse):
			...
			self.RGB_des, self.F_RGB = self.E_RGB.encode(self.data_RGB, reuse=reuse)
			self.E_NIR = Encoder('NIR_Encoder')
			self.NIR_des, self.F_NIR = self.E_NIR.encode(self.data_NIR, reuse=reuse)
		if self.equivariance:
			self.RGB_des = batch_rotate_p4(self.RGB_des, -random_rotA)
			self.NIR_des = batch_rotate_p4(self.NIR_des, -random_rotB)

		all_samples = tf.concat([self.RGB_des, self.NIR_des], axis=0)
		...
		for i in range(2 * N):
			...
			softmax = tf.expand_dims(- pos + tf.reduce_logsumexp(negs, axis=0), axis=-1)
			if i==0:
				softmaxes = softmax
			else:
		...
		self.decay_loss = activation_decay([self.RGB_des, self.NIR_des], p=2.) + activation_decay(
			[self.RGB_des, self.NIR_des], p=1.)
