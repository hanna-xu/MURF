...
class Des_Extract_Model(object):
	...
	def des(self, PET, MRI):
		self.PET = PET # tf.compat.v1.placeholder(tf.float32, shape=(BATCH_SIZE, INPUT_H, INPUT_W, 3), name='PET')
		self.MRI = MRI # tf.compat.v1.placeholder(tf.float32, shape=(BATCH_SIZE, INPUT_H, INPUT_W, 1), name='NMRI')
		oh = MRI.shape[1]
		ow = MRI.shape[2]
		...
		if self.equivariance:
			...
			self.data_PET = batch_rotate_p4(PET, random_rotA)
			self.data_MRI = batch_rotate_p4(MRI, random_rotB)
		else:
			self.data_PET = PET
			self.data_MRI = MRI
		...
		with tf.compat.v1.variable_scope('des_extract'):
			self.E_PET = Encoder('PET_Encoder')
			self.PET_des, self.F_PET = self.E_PET.encode(self.data_PET, reuse=False)
			self.E_MRI = Encoder('MRI_Encoder')
			self.MRI_des, self.F_MRI = self.E_MRI.encode(self.data_MRI, reuse=False)


		if self.equivariance:
			self.PET_des = batch_rotate_p4(self.PET_des, -random_rotA)
			self.MRI_des = batch_rotate_p4(self.MRI_des, -random_rotB)

		if self.is_training:
			PET_ycbcr = PET2ycbcr(self.PET)
			PET_y = PET_ycbcr[:, :, :, 0:1]

			all_samples = tf.concat([self.PET_des, self.MRI_des], axis=0)
			...
			self.decay_loss = activation_decay([self.PET_des, self.MRI_des], p=2.) + activation_decay([self.PET_des, self.MRI_des], p=1.)
			self.grad_loss = l2_loss(tf.abs(grad(ds(self.PET_des))), tf.abs(grad(ds(self.MRI)))) \
							 + l2_loss(tf.abs(grad(ds(self.MRI_des))), tf.abs(grad(ds(PET_y))))
			self.loss = self.contrast_loss + 0.0001 * self.decay_loss # + 50 * self.affinity_loss
...
def PET2ycbcr(img_PET):
	R = tf.expand_dims(img_PET[:, :, :, 0], axis=-1)
	G = tf.expand_dims(img_PET[:, :, :, 1], axis=-1)
	B = tf.expand_dims(img_PET[:, :, :, 2], axis=-1)
