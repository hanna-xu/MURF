...
class F2M_Model(object):
	...
	def f2m(self, PET, MRI, defor_field=None, re_defor_field_gt=None, label=None, is_training=True):
		self.MRI = MRI
		self.PET = PET
		if is_training:
			with tf.device('/gpu:0'):
				if label == None:
					...
					self.defor_field = tf.multiply(defor_field, label)
				...
				warped_R, _, _, _ = grid_sample(PET[0:self.batchsize, :, :, 0:1], resampling_grid)
				warped_G, _, _, _ = grid_sample(PET[0:self.batchsize, :, :, 1:2], resampling_grid)
				warped_B, _, _, _ = grid_sample(PET[0:self.batchsize, :, :, 2:3], resampling_grid)
				self.dPET = tf.concat([warped_R, warped_G, warped_B], axis=-1)
		else:
			self.dPET = PET

		with tf.device('/gpu:0'):
			self.f2m_net = F2M_network('f2m_net')
			dPET_ycbcr = rgb2ycbcr(self.dPET)
			dPET_y = dPET_ycbcr[0: self.batchsize, :, :, 0:1]
			dPET_cb = dPET_ycbcr[0: self.batchsize, :, :, 1:2]
			dPET_cr = dPET_ycbcr[0: self.batchsize, :, :, 2:3]

			self.dPET_d2=tf.nn.max_pool2d(self.dPET, [1,2,2,1], strides=[1, 2, 2, 1], padding='SAME')
			dPET_ycbcr_d2 = rgb2ycbcr(self.dPET_d2)
			dPET_y_d2 = dPET_ycbcr_d2[0: self.batchsize, :, :, 0:1]
			dPET_cb_d2 = dPET_ycbcr_d2[0: self.batchsize, :, :, 1:2]
			dPET_cr_d2 = dPET_ycbcr_d2[0: self.batchsize, :, :, 2:3]

			PET_ycbcr = rgb2ycbcr(self.PET)
			PET_y = PET_ycbcr[0: self.batchsize, :, :, 0:1]
			PET_cb = PET_ycbcr[0: self.batchsize, :, :, 1:2]
			PET_cr = PET_ycbcr[0: self.batchsize, :, :, 2:3]

			self.mean_img_before = ycbcr2rgb(tf.concat([(self.MRI + PET_y) / 2, PET_cb, PET_cr], axis=-1))
			self.mean_img_after = ycbcr2rgb(tf.concat([(self.MRI + dPET_y) / 2, dPET_cb, dPET_cr], axis=-1))

			self.mean_y_before = (self.MRI + PET_y) / 2
			self.mean_y_after = (self.MRI + dPET_y) / 2

			self.fused_img, self.offset, self.res_PET = self.f2m_net.fuse(self.dPET, self.MRI,is_training=is_training)
			
			if is_training:
				dr_defor_gt, _, _, _ = grid_sample(self.dPET[0:self.batchsize, :, :, 0:1], - (resampling_grid - identity) + identity)
				dg_defor_gt, _, _, _ = grid_sample(self.dPET[0:self.batchsize, :, :, 1:2], - (resampling_grid - identity) + identity)
				db_defor_gt, _, _, _ = grid_sample(self.dPET[0:self.batchsize, :, :, 2:3], - (resampling_grid - identity) + identity)
				self.dPET_defor_gt = tf.concat([dr_defor_gt, dg_defor_gt, db_defor_gt], axis=-1)
				
				fimg_ycbcr = rgb2ycbcr(self.fused_img)
				fimg_y = fimg_ycbcr[0: self.batchsize, :, :, 0:1]
				fimg_cb = fimg_ycbcr[0: self.batchsize, :, :, 1:2]
				fimg_cr = fimg_ycbcr[0: self.batchsize, :, :, 2:3]
				self.fimg_y = fimg_y
	
				A = tf.reduce_min(self.fimg_y)
				B = tf.reduce_max(self.fimg_y)
				self.fimg_y = 1/(B-A) * self.fimg_y + A/(A-B)

				self.fused_img_d2=tf.nn.max_pool2d(self.fused_img, [1,2,2,1], strides=[1, 2, 2, 1], padding='SAME')
				fimg_ycbcr_d2 = rgb2ycbcr(self.fused_img_d2)
				fimg_y_d2 = fimg_ycbcr_d2[0: self.batchsize, :, :, 0:1]
				fimg_cb_d2 = fimg_ycbcr_d2[0: self.batchsize, :, :, 1:2]
				fimg_cr_d2 = fimg_ycbcr_d2[0: self.batchsize, :, :, 2:3]
				self.fimg_y_d2 = fimg_y_d2
	
				dPET_offset_ycbcr = rgb2ycbcr(self.res_PET)
				dPET_offset_y = dPET_offset_ycbcr[0: self.batchsize, :, :, 0:1]
				dPET_offset_cb = dPET_offset_ycbcr[0: self.batchsize, :, :, 1:2]
				dPET_offset_cr = dPET_offset_ycbcr[0: self.batchsize, :, :, 2:3]

				self.sim_loss = 1 - 0.5 * SSIM(fimg_y, self.MRI) - 0.5 * SSIM(fimg_y, dPET_y)
	
				enhanced_gradient_dPET_y = tf.where(gradient(dPET_y) > tf.zeros_like(gradient(dPET_y)),
													tf.pow(gradient(dPET_y), 0.7), -tf.pow(-gradient(dPET_y), 0.7))
				enhanced_gradient_dPET_offset_y = tf.where(gradient(dPET_offset_y) > tf.zeros_like(gradient(dPET_offset_y)),
													tf.pow(gradient(dPET_offset_y), 0.7), -tf.pow(-gradient(dPET_offset_y), 0.7))
				enhanced_gradient_MRI = tf.where(gradient(self.MRI) > tf.zeros_like(gradient(self.MRI)),
												tf.pow(gradient(self.MRI), 0.7), -tf.pow(-gradient(self.MRI), 0.7))
				self.max_gradient_loss = l1(gradient(self.fimg_y), tf.where(tf.abs(gradient(dPET_offset_y)) > tf.abs(gradient(self.MRI)),
													enhanced_gradient_dPET_offset_y,  enhanced_gradient_MRI))

				self.fuse_loss = self.sim_loss + 10 * self.max_gradient_loss
				self.smooth_loss = tf.reduce_mean(tf.abs(gradient(self.offset[:,:,:,0:1])) + tf.abs(gradient(self.offset[:,:,:,1:2])))
				self.fimg_grad_loss = l0(self.fimg_y)
				self.grad_diff_loss = tf.reduce_mean(tf.abs(gradient_1(dPET_offset_y) - gradient_1(PET_y)))
				self.defor_diff = l2(self.re_defor_field_gt, self.offset)
				self.offset_loss = self.defor_diff
...
class F2M_network(object):
	...
	def fuse(self, img_a, img_b,reuse=False, is_training=False):
		with tf.variable_scope(self.scope, reuse=reuse):
			a_offset, self.offset = convoffset2D(img_a, img_b, scope_name='conv1_offset',is_training=is_training)

			weights = tf.compat.v1.get_variable("w1", [1, 1, 3, 8],
												initializer=tf.truncated_normal_initializer(stddev=WEIGHT_INIT_STDDEV))
			bias = tf.compat.v1.get_variable("b1", [8], initializer=tf.constant_initializer(0.0))


			conv_a1 = tf.nn.conv2d(a_offset, weights, strides=[1, 1, 1, 1], padding='SAME') + bias
			conv_a1 = lrelu(conv_a1)
			conv_b1 = tf.nn.conv2d(tf.tile(img_b, [1, 1, 1 ,3]), weights, strides=[1, 1, 1, 1], padding='SAME') + bias
			conv_b1 = lrelu(conv_b1)

			weights = tf.compat.v1.get_variable("w2", [3, 3, 8, 16],
												initializer=tf.truncated_normal_initializer(stddev=WEIGHT_INIT_STDDEV))
			...
			weights = tf.compat.v1.get_variable("w3", [3, 3, 16, 32],
												initializer=tf.truncated_normal_initializer(stddev=WEIGHT_INIT_STDDEV))
			bias = tf.compat.v1.get_variable("b3", [32], initializer=tf.constant_initializer(0.0))
			conv_a3 = tf.nn.conv2d(conv_a2, weights, strides=[1, 1, 1, 1], padding='SAME') + bias
			conv_a3 = lrelu(conv_a3)
			# conv_a3 = tf.concat([conv_a3, a_offset1_grad], axis=-1)
			...
			fused = conv_a3 + conv_b3  # tf.where(conv_a3>conv_b3, conv_a3, conv_b3)
			...
			weights = tf.compat.v1.get_variable("wf3", [3, 3, 16, 3],
												initializer=tf.truncated_normal_initializer(stddev=WEIGHT_INIT_STDDEV))
			bias = tf.compat.v1.get_variable("bf3", [3], initializer=tf.constant_initializer(0.0))
			x = tf.nn.conv2d(x, weights, strides=[1, 1, 1, 1], padding='SAME') + bias
...
def convoffset2D(x, y, scope_name=None, is_training=False):
	x_shape = tf.shape(x)
	...
	with tf.variable_scope(scope_name):
		with tf.variable_scope('offset_conv'):
			...
			offsets1 = tf.contrib.layers.batch_norm(inputs=offsets1,is_training=is_training, activation_fn=None)
			...
			offsets2 = tf.contrib.layers.batch_norm(inputs=offsets2,is_training=is_training, activation_fn=None)
			...
			offsets3 = tf.contrib.layers.batch_norm(inputs=offsets3,is_training=is_training, activation_fn=None)
			...
			offsets5 = conv(offsets3_ds, channels=32, kernel=3, stride=1, pad=1, pad_type='reflect', scope='conv5',
							use_bias=True)
			offsets5 = tf.contrib.layers.batch_norm(inputs=offsets5,is_training=is_training, activation_fn=None)
			...
			offsets6 = tf.contrib.layers.batch_norm(inputs=offsets6,is_training=is_training, activation_fn=None)
			...
			offsets7 = tf.contrib.layers.batch_norm(inputs=offsets7,is_training=is_training, activation_fn=None)
			...
			offsets8 = tf.contrib.layers.batch_norm(inputs=offsets8,is_training=is_training, activation_fn=None)
			...
			offsets10 = tf.contrib.layers.batch_norm(inputs=offsets10,is_training=is_training, activation_fn=None)
