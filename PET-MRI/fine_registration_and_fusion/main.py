...
def main():
	with tf.Graph().as_default(), tf.compat.v1.Session() as sess:
		...
		model.f2m(SOURCE_PET, SOURCE_MRI, DEFOR_FIELD,RE_DEFOR_FIELD_GT,is_training=True)
		...
		dPET = tf.compat.v1.summary.image('dPET', model.dPET, max_outputs=NUM)
		diff1 = tf.compat.v1.summary.image('PET-MRI', model.mean_img_before, max_outputs=NUM)
		fimg = tf.compat.v1.summary.image('fused_img', model.fused_img, max_outputs=NUM)
		fimgy = tf.compat.v1.summary.image('fused_img_y',model.fimg_y,max_outputs=NUM)
		dgt = tf.compat.v1.summary.image('defor_gt', model.dPET_defor_gt, max_outputs=NUM)
		merge_summary1 = tf.compat.v1.summary.merge(
			[SOURCE1, SOURCE2, gloss, dPET, diff1,fimg, fimgy, loss1, loss2, dgt])
		...
		train_f2m(model=model, sess=sess, trainset=source_data, merged1=merge_summary1, writer=writer1,
